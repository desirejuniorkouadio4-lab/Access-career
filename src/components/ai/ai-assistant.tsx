          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-3 space-y-3 min-h-[200px] max-h-[350px]">
            {messages.length === 0 && (
              <div className="text-center py-6">
                <Bot size={28} className="text-violet-300 mx-auto mb-3" />
                <p className="text-sm font-semibold text-ink">Comment puis-je vous aider ?</p>
                <p className="text-xs text-zinc-400 mt-1 mb-4">Questions sur &ldquo;{lessonTitle}&rdquo;</p>
                {["Explique-moi cette leçon", "Donne-moi un exemple concret", "Quels sont les points clés ?"].map(q => (
                  <button key={q} onClick={() => setInput(q)}
                    className="block w-full text-left px-3 py-2 mb-1.5 bg-violet-50 text-violet-700 text-xs font-medium rounded-xl hover:bg-violet-100 transition">
                    {q}
                  </button>
                ))}
              </div>
            ...
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[85%] px-3.5 py-2.5 rounded-2xl text-sm leading-relaxed ${
                  m.role === "user" ? "bg-violet-700 text-white rounded-br-md" : "bg-zinc-100 text-ink rounded-bl-md"
                }`}>{m.content}</div>
              </div>
            ))}
            {loading && (
